...
import { acilisSaatiYukle, durum } from './acilisSaati'

const tarih = (gun: number, saat: number, dakika = 0) => new Date(2026, 8, gun, saat, dakika)

describe('açılış saati durumu', () => {
  it('yüklenmeden önce durumu bilinmiyor döndürür', () => {
    expect(durum('Mo-Su 09:00-17:00', tarih(21, 10), 60)).toEqual({ durum: 'bilinmiyor', saat: null })
  })

  it('her gün açık, kapalı ve kalışta kapanacak durumları ayırır', async () => {
    await acilisSaatiYukle()
  ...
  it('pazartesi kapalı kuralını ve ertesi günün açılışını kullanır', async () => {
    await acilisSaatiYukle()
  ...
  it('24/7 ve çözülemeyen metinde uyarı üretmez', async () => {
    await acilisSaatiYukle()
